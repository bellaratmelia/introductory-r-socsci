...
### Example: with 1 continuous predictor

**Research Question:** Does a person's age influence their income in Chile?

The outcome/DV ($Y$) here is `income`, while the predictor/IV ($X$) would be `age`

```{r}
#| echo: true
income_model1 <- lm(income ~ age, data = chile_data)
summary(income_model1) #summarize the result
```
...
### Example: with multiple continuous predictors

**Research Question:** Do a person's age and their support for the status quo (statusquo) have an impact on their income?

The outcome/DV ($Y$) here is `income`, while the predictors/IVs ($X_1$ and $X_2$) would be `age` and `statusquo`

```{r}
#| echo: true
income_model2 <- lm(income ~ age + statusquo, data = chile_data)
summary(income_model2)
```

------------------------------------------------------------------------

### Using huxtable to present both models in your paper
...
#| echo: true
library(huxtable)
huxreg("age" = income_model1, "age + statusquo" = income_model2)
```

------------------------------------------------------------------------

### Heads-up: Multicollinearity
...
**Research question**: Explore the difference of income between different education levels.
...
```{r}
#| echo: true
income_modelcat <- lm(income ~ education, data = chile_data)
summary(income_modelcat)
```
...
### Linear Regression group exercise (solo attempts ok)

Create a regression model called `statusquo_model` that predicts the `statusquo` score based on `age` and `education`. Make sure that the reference category for `education` is set to **P** (Primary).
...
statusquo_model <- lm(statusquo ~ age + education, data = chile_data)
summary(statusquo_model)
```
...
### Example: with 1 categorical Predictor
...
### Logistic Regression group exercise (solo attempts ok)
...
## If you'd like to continue your learning:

These are the resources that I would recommend:

-   [R for Data Science](https://r4ds.hadley.nz/) by Hadley Wickham (free) - one of the most popular free R resource (I've used this myself!)

-   [Statistical Inference via Data Science: A ModernDive into R and the Tidyverse](https://www.tidymodels.org/books/moderndive/) by Chester Ismay and Albert Y. Kim (free, print book available for purchase) - Very gentle introduction to tidyverse, regression, and statistical inference.

-   [A Guide on Data Analysis](https://bookdown.org/mike/data_analysis/) by Mike Nguyen (also free) - covers different kinds of regression and quasi-experimental designs such as Regression Discontinuity Design (RDD), among others.

-   [Big Book of R](https://www.bigbookofr.com/) by Oscar Baruffa (also vv free) - Aggregation of many free resources on R in various topics (e.g. archaeology, people analytics, sports analytics, etc.)
