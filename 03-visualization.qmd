...
-   Descriptive Statistics: single variable, and two variables.

-   Basic R functions for descriptive statistics e.g., mean(), median(), sd(), var(), range(): range of values, IQR(), summary(), and Mode().

-   Use ggplot to build charts: ggplot(data, aes(x, y)). Begin a plot that you finish by adding layers.

-   Use a geom function to represent data. Add one geom function per layer.

-   Use position adjustments determine how to arrange geoms that would otherwise occupy the same space.

-   Use labs() to label the elements of your plot.

-   Use facets divide a plot into subplots based on the values of one or more discrete variables e.g., facet_grid().

-   Use ggsave() to save your images.

-   Choose an appropriate visualization to tell the story of the data, e.g., choose graphs to present different numbers of variables, different types of data (continuous and non-continuous).
